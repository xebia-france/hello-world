<template>
  <div class="hello">
    ...
    <h1 v-else-if="hello_count === 1">Only {{ hello_count }} person says hello !</h1>
    <h1 v-else-if="hello_count === 0">Nobody said hello !</h1>
    <md-button class="md-raised md-primary" @click.native="addHello">Say hello!</md-button>
    <md-button class="md-raised md-primary" @click.native="clearHello">Reset</md-button>
  </div>
</template>
...
export default {
  name: 'hello',
  mounted () {
    this.$nextTick(this.fetchHellos)
  },
  methods: {
    clearHello () {
    ...
    fetchHellos () {
      ...
      return axios.get('http://localhost:8081/hello')
        .then(resp => {
          that.hello_count = resp.data.hello_count
        })
    },
    addHello () {
